...
//    Page<Order> findAllByStatusIsTrueOrderByIdDesc(Pageable pageable);
    @Query(value = "select * from orders where book_size > 0 and (person_id in (select id from person where concat(name,code, phone) like :key)) and id in (select order_id from order_detail where status = true)",nativeQuery = true)
...
@Query(value = "select * from orders where book_return > 0 and (person_id in (select id from person where concat(name,code, phone) like :key)) and id in (select order_id from order_detail where status = true)",nativeQuery = true)
